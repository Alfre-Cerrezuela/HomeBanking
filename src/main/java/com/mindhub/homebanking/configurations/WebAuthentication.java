...
public class WebAuthentication  extends GlobalAuthenticationConfigurerAdapter {
    ...
    public void init(AuthenticationManagerBuilder authentication) throws Exception{
        authentication.userDetailsService(inputEmail -> {

            Client client = clientRepository.findByEmail(inputEmail);

            if(client != null){
                if(client.getEmail().contains("admin") && client.getEmail().contains("procariota")){
                ...
                throw new UsernameNotFoundException("Unknow client:" + inputEmail);
            }
        });
    }
